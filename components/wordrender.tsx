...
export default async function WordRender({word}: Props) {
  let wordData;
  try {
    wordData = await wordService.getWordData(word);
  } catch (error) {
    return <div className="text-center p-12 text-xl text-red-400">Failed to generate word data.</div>;
  }
  const { mainWord, synonyms, antonyms, similar } = wordData;
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-white tracking-tight glitch-effect">
                    {mainWord.word}
                  </h1>
                ...
                <div className="flex items-center gap-3 text-xl sm:text-2xl md:text-3xl font-medium text-muted-foreground mt-4 md:mt-0">
                  <ArrowRight className="w-5 h-5 sm:w-6 sm:h-6 text-primary animate-pulse" />
